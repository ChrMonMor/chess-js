...
function legalMoveHighlight(origin, coor, colour) {
    ...
    if (isTheKingInDanger) {
        if (legalDiv.querySelector('#King_Black') !== null || legalDiv.querySelector('#King_White') !== null) {
            hitList.push(origin);
        }
    }
}
...
function movePieceToLegal(origin, coor) {
    ...
    checkForCheckOrMate(whomest, true);
    checkForCheckOrMate(whoIsTheEnemy(whomest), false);
}
...
function pawnAttackN(coor, colour, highlight) {
    if (Number(coor[0]) != 8) {
...
function pawnAttackS(coor, colour, highlight) {
    if (Number(coor[0]) != 1) {
        let xy = Number(Number(coor[0]) - 1) + ',' + Number(Number(coor[2]) - 1);
        let answer = isThereGamePieceHere(xy, colour);
        if (answer || highlight) {
...
function checkIfKingIsDead(colour) {
    ...
    if (typeof (king) == 'undefined' || king == null) {
        winnerMessage(colour);
    }
}

function winnerMessage(colour) {
...
function checkIfkingCouldDieIfMoved(coor, colour) {
    highlightAllPosibleMovesForEnemy(colour);
...
function highlightAllPosibleMovesForEnemy(colour) {
    var enemy = whoIsTheEnemy(colour);
    ...
    for (let index = 0; index < enemyArray.length; index++) {
        ...
        if (piece[0] == "Pawn") {
            if (colour == "White") {
                pawnAttackS(enemyArray[index].parentElement.id, enemy, true);
            } else {
                pawnAttackN(enemyArray[index].parentElement.id, enemy, true);
            }
        }
    }
...
function checkForCheckOrMate(colour, yourTurn) {
    highlightAllPosibleMovesForEnemy(colour);
    var king = document.getElementById('King_' + colour);
    var gridcell = king.parentNode;
    // here we learn if someone can see the king 
    logic: {
        if (gridcell.querySelectorAll('.legalmove').length > 0) {
            removeLegalMoves();
            checkIfkingCouldDieIfMoved(gridcell.id, colour);
            var dot = document.getElementsByClassName("legalmove");
            // here we learn if the king can move
            if (!yourTurn) {
                document.getElementsByTagName('h1')[0].innerHTML += " - Checkmate!";
                winnerMessage(whomest);
                whomest = whoIsTheEnemy(whomest);
                break logic;
            }
            if (dot.length > 0) {
                document.getElementsByTagName('h1')[0].innerHTML += " - Check!";
            } else {
                // here we learn if someone can be sacrificed or kill the future killer
                isTheKingInDanger = true;
                highlightAllPosibleMovesForEnemy(colour);
                console.log(hitList);
                if (condition) {
